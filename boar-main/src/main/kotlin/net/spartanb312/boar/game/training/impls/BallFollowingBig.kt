...
class BallFollowingBig(scoreboardScreen: ScoreboardScreen, scene: Scene) : BallHitTraining(
    scoreboardScreen,
    scene,
    1,
    2f.asRange,
    ...
    companion object : TrainingInfo("Following Big", "") {
        override fun new(scoreboardScreen: ScoreboardScreen, scene: Scene): Training {
            return BallFollowingBig(scoreboardScreen, scene)
        }
    }
    ...
    override fun onTick() {
        ...
        click()
        entities.forEach {
            if (it is Ball) it.randomMove(reverse, 4f)
        }
    }
